 ...
 */
public class RAQuadTreeDistanceV2 implements IAlgorithm {
...
//                    System.out.println("[queue] sample size = " + sampleSize);
    ...
    public byte[] answerQuery(Query query) {
        ...
        if (!this.finish) {
            ...
            System.out.println("[RA-QuadTree-DistanceV2] has not finished loading data, select samples temporarily for progressive results!");
            MyTimer.startTimer();
            this.quadTree.selectSamples(0.5, 0.5, 0.5, 0);
            MyTimer.stopTimer();
            double selectSamplesTime = MyTimer.durationSeconds();
